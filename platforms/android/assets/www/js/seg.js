...
var app = {
    ...
    initialize: function() {
      ...
      $('#label_topo').html((localStorage.getItem('typeSeg')=='seguidores'?'Seguidores':'Seguindo'))
    },
...
function loadCurtidas(){
  $.ajax({
    ...
    data: {
        'q': 'quem_'+localStorage.getItem('typeSeg'),
        'idusuario': localStorage.getItem('viewProfile'),
        'limit': limit
    },
    async: true,
    dataType: "json",
    success: function (json) {
      ...
      if (json[0]['res'] == 1){
        ...
        $('#noti').append('<div style="text-align:center;margin-top:10%;font-size:1.5em;width:80%;margin-left:10%;font-family:Lato-Light;">'+(localStorage.getItem('typeSeg')=='seguindo'?'Não está seguindo ninguém.':'Não há seguidores.')+'</div>');
      }
