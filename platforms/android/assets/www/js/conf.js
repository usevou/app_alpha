...
var app = {
    ...
    initialize: function() {
      ...
      $('#label_topo').html(localStorage.getItem('viewConf'));
    },
...
function loadCurtidas(){
  $.ajax({
    ...
    data: {
        ...
        'tipo': (localStorage.getItem('viewConf')=='Confirmados'?'1':(localStorage.getItem('viewConf')=='Check-in'?'3':'0')),
        'limit': limit
    },
    async: true,
    dataType: "json",
    success: function (json) {
      ...
      if (json[0]['res'] == 1){
        ...
        $('#noti').append('<div style="text-align:center;margin-top:10%;font-size:1.5em;width:80%;margin-left:10%;font-family:Lato-Light;">Ninguém '+(localStorage.getItem('viewConf')=='Confirmados'?'confirmou presença n':(localStorage.getItem('viewConf')=='Check-in'?'fez check-in n':'foi convidado para '))+'esse evento.</div>');
      }
